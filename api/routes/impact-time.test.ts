...
describe('GET /api/impact-time', () => {
  test('returns 400 when required params are missing', async () => {
    const res = await api.request('/api/impact-time', {}, await createTestEnv());
    expect(res.status).toBe(400);
  });

  test('returns the documented response shape', async () => {
    const res = await api.request(`/api/impact-time${baseQuery}`, {}, await createTestEnv());
  ...
  test('routes the query to the requested instance', async () => {
    await api.request(`/api/impact-time${baseQuery}`, {}, await createTestEnv());
