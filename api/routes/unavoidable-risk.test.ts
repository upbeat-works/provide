...
describe('GET /api/unavoidable-risk', () => {
  test('returns 400 when required params are missing', async () => {
    const res = await api.request('/api/unavoidable-risk', {}, await createTestEnv());
  ...
  test('routes the query to the requested instance', async () => {
    await api.request(`/api/unavoidable-risk${baseQuery}`, {}, await createTestEnv());
  ...
  test('returns 404 when the instance slug is unknown', async () => {
    const res = await api.request('/api/unavoidable-risk?indicator=x&geography=DEU&scenarios=curpol&instance=does-not-exist', {}, await createTestEnv());
