<script>
  import { LABEL_DOCUMENTATION, LABEL_CASE_STUDIES, LABEL_CONTACT, LABEL_ABOUT, PATH_ABOUT, PATH_CONTACT, PATH_DOCUMENTATION, PATH_EXPLORE, PATH_IMPACT, LABEL_EXPLORE, PATH_CASE_STUDIES, TITLE_PROJECT, TITLE_SITE } from '$config';
...
<footer class="bg-surface-weaker border-t border-contour-weakest">
    ...
    <div class="flex flex-col md:flex-row md:items-start md:justify-between gap-8">
      ...
      <nav>
        <ul class="flex flex-wrap gap-x-6 gap-y-2">
          ...
          <li><a class="text-sm text-theme-stronger hover:underline transition-colors" href="/{PATH_CASE_STUDIES}">{LABEL_CASE_STUDIES}</a></li>
  ...
  <!-- Bottom Bar -->
  <div class="bg-petrol-50 py-3">
    <div class="text-sm mx-auto max-w-7xl px-6 flex justify-between">
      <span class="text-contour-weak">IIASA <time datetime={new Date().getFullYear()}>{new Date().getFullYear()}</time></span>
