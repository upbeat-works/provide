...
test('turns local config into labelled choices and retains valid URL values', () => {
  ...
  const result = load({
    ...
    url: new URL(
      'https://example.test/impacts/eu-scoreboard/indicators?sector=testing&indicator=Mean%20Air%20Temperature&scenario=1.5C&region=Ukraine&year=2100'
    ),
  });
...
test('defaults the country and year without a discovery request', () => {
  ...
  const result = load({ data: { scoreboard, scenarioLabels: {} }, url: new URL('https://example.test/impacts/eu-scoreboard/indicators') });
