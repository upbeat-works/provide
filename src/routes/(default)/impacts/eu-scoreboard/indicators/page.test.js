...
test('writes the indicator parameter without changing the sector', async () => {
  render(Page, {
    data: common,
    url: new URL('http://localhost/impacts/eu-scoreboard/indicators?sector=testing&indicator=Maximum%20Air%20Temperature'),
  });
...
test.each([false, true])('changes sector and keeps the full indicator selection while comparison is %s', async (comparing) => {
  ...
  render(Page, {
    ...
    url: new URL(
      'http://localhost/impacts/eu-scoreboard/indicators?sector=testing&indicator=Maximum%20Air%20Temperature&region=Austria&scenario=scenario&year=2050'
    ),
  });
  if (comparing) {
...
test('keeps charts and country context available for a sector without a regional map', async () => {
  ...
  render(Page, {
    ...
    url: new URL('http://localhost/impacts/eu-scoreboard/indicators?sector=testing&region=Austria&scenario=scenario&year=2050'),
  });
